...
class AttentionModel(nn.Module):
    ...
    def forward(
        ...
        embeddings_init, init_context, attn, V, h_old = self.embedder(
        ...
        if self.is_op:
            loc_with_depot = torch.cat((input["depot"][:, None, :], input["loc"]), 1)
            max_length = input["max_length"][:, None] - (
                loc_with_depot[:, 0:1] - loc_with_depot
            ).norm(p=2, dim=-1)
            prize_with_depot = torch.cat(
                (torch.zeros_like(input["prize"][:, :1]), input["prize"]), 1
            )
        if self.is_pctsp:
            loc_with_depot = torch.cat((input["depot"][:, None, :], input["loc"]), 1)
            penalty_with_depot = torch.cat(
                (torch.zeros_like(input["penalty"][:, :1]), input["penalty"]), 1
            )
            expected_prize = input["deterministic_prize"]
            real_prize = input[
                "stochastic_prize" if self.stochastic else "deterministic_prize"
            ]
        ...
        for i in range(self.n_paths):
            seq_beam = torch.zeros(
                batch_size, 1, 1, dtype=torch.long, device=embeddings_init.device
            )
            ...
            mask_beam = (
                torch.zeros(
                    ...
                    graph_size,
                    dtype=torch.uint8,
                    device=embeddings_init.device,
                )
                > 0
            )
            if self.is_vrp:
                used_beam = demand.new_zeros(batch_size, 1)
                visited_beam = torch.zeros(
                    batch_size,
                    1,
                    graph_size,
                    dtype=torch.uint8,
            ...
            if self.is_op:
                ...
                coord_cur = loc_with_depot[
                    torch.arange(batch_size).view(-1, 1), seq_beam[:, :, -1]
                ].view(batch_size, seq_beam.size(1), 1, -1)
                mask_beam = (
                    (coord_all - coord_cur).norm(p=2, dim=-1)
                ) > max_length.view(batch_size, 1, -1)
            if self.is_pctsp:
                visited_beam = torch.zeros(
                    batch_size,
                    1,
            ...
            while (
                (
                    ...
                    and (
                        j == 0
                        or not (seq_beam[:, :, -1][ll_beam != float("-inf")] == 0).all()
                    )
                )
                or (
                    not self.is_vrp
                    and not self.is_op
                    and not self.is_pctsp
                    and not mask_beam[ll_beam != float("-inf")].all()
                )
                or (
                    self.is_vrp
                    and not self.allow_partial
                    and not visited_beam[ll_beam != float("-inf")].all()
                )
                or (
                    self.is_vrp
                    and self.allow_partial
                    and (
                        j < loc_with_depot.size(1)
                        or (demand_with_depot[ll_beam != float("-inf")] > 0).any()
                    )
                )
                or (
                    self.is_pctsp
                ...
                for beam_index in range(ll_beam.size(1)):
                    ...
                    fixed = fixed_beam[
                        beam_index * batch_size : (beam_index + 1) * batch_size
                    ]
                    if self.is_vrp:
                        if self.allow_partial:
                            ...
                            log_p = self._get_log_p(
                                fixed, mask, seq, i, used=used_beam[:, beam_index]
                            )[:, 0, :]
                        # log_p[(ll_beam[:, beam_index]!=float("-inf")) & (visited_beam[:, beam_index].all(dim=-1)), 0] = 0
                    elif self.is_op:
                        remaining_beam = (
                            input["max_length"].view(batch_size, 1)
                            - distance_beam[:, beam_index]
                        )

                        log_p = self._get_log_p(
                            fixed, mask, seq, i, remaining=remaining_beam
                        )[:, 0, :]
                    elif self.is_pctsp:
                        remaining_beam = torch.clamp(
                            1 - prize_beam[:, beam_index, None], min=0
                        )
                ...
                if self.is_vrp or self.is_op or self.is_pctsp:
                    distance_beam = distance_beam + (
                        loc_with_depot[
                            torch.arange(batch_size).view(-1, 1), seq_beam[:, :, -1]
                        ].view(batch_size, n_beam, 1, -1)
                        - loc_with_depot[
                            torch.arange(batch_size).view(-1, 1), node_index_beam
                        ].view(batch_size, n_beam, expand_size, -1)
                    ).norm(p=2, dim=-1)
                else:
                    distance_beam = distance_beam + (
                        input[
                ...
                if self.is_vrp:
                    if self.allow_partial:
                        ...
                        selected_demand = demand_with_depot.view(-1, graph_size)[
                            torch.arange(batch_size * demand_with_depot.size(1)).view(
                                -1, 1
                            ),
                            node_index_beam.view(
                                batch_size * demand_with_depot.size(1), -1
                            ),
                        ].view(batch_size, -1)
                    else:
                        selected_demand = demand[
                            torch.arange(batch_size).view(-1, 1),
                            torch.clamp(node_index_beam - 1, 0, graph_size - 1),
                        ]
                    used_beam = torch.min(
                        (
                            used_beam.view(batch_size, n_beam, 1)
                            .expand(batch_size, n_beam, expand_size)
                            .contiguous()
                            .view(batch_size, -1)
                            + selected_demand
                        )
                        * (node_index_beam != 0).float(),
                        self.problem.VEHICLE_CAPACITY
                        * torch.ones_like(selected_demand),
                    )
                if fst == 1 and j > 0:
                    ...
                    if self.is_vrp or self.is_op or self.is_pctsp:
                        if self.allow_partial:
                            b_index = (
                                demand_with_depot.view(batch_size, 1, n_beam, -1)
                                == demand_with_depot.view(batch_size, n_beam, 1, -1)
                            ).all(dim=-1)
                        else:
                            b_index = (
                                visited_beam.view(batch_size, 1, n_beam, -1)
                                == visited_beam.view(batch_size, n_beam, 1, -1)
                            ).all(dim=-1)
                        b_index[:, torch.arange(n_beam), torch.arange(n_beam)] = 0

                        b_index = b_index.view(batch_size, n_beam, 1, n_beam, 1).expand(
                            -1, -1, expand_size, -1, expand_size
                        )
                        b_index = b_index.contiguous().view(
                            batch_size, n_beam * expand_size, n_beam * expand_size
                        )
                        n_index = node_index_beam.view(
                            batch_size, 1, -1
                        ) == node_index_beam.view(batch_size, -1, 1)
                        n_index[
                            :,
                            torch.arange(n_beam * expand_size),
                            torch.arange(n_beam * expand_size),
                        ] = 0
                        bn_index = b_index & n_index

                        index1 = bn_index & (
                            distance_beam.view(batch_size, n_beam * expand_size, 1)
                            >= distance_beam.view(batch_size, 1, n_beam * expand_size)
                        )
                        index1[torch.tril(index1 & index1.transpose(-2, -1))] = 0
                        if self.is_vrp:
                            index1 = index1 & (
                                used_beam.view(batch_size, n_beam * expand_size, 1)
                                >= used_beam.view(batch_size, 1, n_beam * expand_size)
                            )
                    else:
                        b_index = (
                            mask_beam.view(batch_size, 1, n_beam, -1)
                            == mask_beam.view(batch_size, n_beam, 1, -1)
                        ).all(dim=-1)
                        b_index[:, torch.arange(n_beam), torch.arange(n_beam)] = 0
                        f_index = seq_beam[:, :, 1].view(batch_size, 1, -1) == seq_beam[
                            :, :, 1
                        ].view(batch_size, -1, 1)
                    ...
                    ll_beam_new1[index11] = l_t + torch.max(
                        (
                            ll_beam_new.view(batch_size, n_beam * expand_size, 1)
                            + index2
                        ).max(-2)[0][index11],
                        ll_beam_new[index11],
                    )
                ...
                ll_beam, top_index = torch.topk(
                    ll_beam_new, min(beam_size, ll_beam_new.size(1))
                )
                node_to_add = node_index_beam.gather(1, top_index)

                mask_beam = mask_beam[
                    torch.arange(batch_size).view(-1, 1), top_index // expand_size
                ]
                seq_beam = seq_beam[
                    torch.arange(batch_size).view(-1, 1), top_index // expand_size
                ]
                seq_beam = torch.cat(
                    [seq_beam, node_to_add.view(batch_size, -1, 1)], dim=-1
                )

                distance_beam = distance_beam[
                    torch.arange(batch_size).view(-1, 1), top_index
                ]
                ...
                if j > 1 and j % n_EG == 0:
                    ...
                    for beam_index in range(mask_beam.size(1)):
                        ...
                        if self.is_pctsp:
                            mask_attn[
                                torch.arange(batch_size), seq_beam[:, beam_index, 0]
                            ] = (
                                mask_attn[
                                    torch.arange(batch_size), seq_beam[:, beam_index, 0]
                                ]
                    ...
                    fixed_beam = AttentionModelFixed(
                        node_embeddings=torch.cat(
                            [fixed.node_embeddings for fixed in fixed_beam], dim=0
                        ),
                        context_node_projected=torch.cat(
                            [fixed.context_node_projected for fixed in fixed_beam],
                            dim=0,
                        ),
                        glimpse_key=torch.cat(
                            [fixed.glimpse_key for fixed in fixed_beam], dim=1
                        ),  # dim 0 are the heads
                        glimpse_val=torch.cat(
                            [fixed.glimpse_val for fixed in fixed_beam], dim=1
                        ),  # dim 0 are the heads
                        logit_key=torch.cat(
                            [fixed.logit_key for fixed in fixed_beam], dim=0
                        ),
                    )

                else:
                    fixed_index = top_index // expand_size * batch_size + torch.arange(
                        batch_size, device=top_index.device
                    ).view(-1, 1)
                    fixed_index = (
                        fixed_index.transpose(dim0=1, dim1=0).contiguous().view(-1)
                    )
                    fixed_beam = fixed_beam[fixed_index]

                if self.is_op:
                    visited_beam = visited_beam[
                        torch.arange(batch_size).view(-1, 1), top_index // expand_size
                    ]
                    ...
                    exceeds_length = (
                        distance_beam.view(batch_size, -1, 1)
                        + (coord_all - coord_cur).norm(p=2, dim=-1)
                    ) > max_length.view(batch_size, 1, -1)
                    ...
                    prize_beam = prize_beam[
                        torch.arange(batch_size).view(-1, 1), top_index // expand_size
                    ]
                    prize_to_collect = real_prize_with_depot[
                        torch.arange(batch_size).view(-1, 1), seq_beam[:, :, -1]
                    ]
                    ...
                    mask_beam[:, :, 0] = (prize_beam < 1.0) & (
                        visited_beam[:, :, 1:].int().sum(-1)
                        < visited_beam[:, :, 1:].size(-1)
                    )
                    ...
                    delivered_demand = torch.min(
                        selected_demand, self.problem.VEHICLE_CAPACITY - used_beam_last
                    )
                    demand_with_depot = demand_with_depot.scatter(
                        -1,
                        seq_beam[:, :, -1, None],
                        demand_with_depot.gather(-1, seq_beam[:, :, -1, None])
                        - delivered_demand[:, :, None],
                    )

                    mask_loc = (demand_with_depot[:, :, 1:] == 0) | (
                        used_beam[:, :, None] >= self.problem.VEHICLE_CAPACITY
                    )
                    mask_depot = (seq_beam[:, :, -1] == 0) & (
                    ...
                    mask_beam = (visited_beam[:, :, 1:] > 0) | (
                        demand.view(batch_size, 1, graph_size - 1)
                        + used_beam.view(batch_size, -1, 1)
                        > self.problem.VEHICLE_CAPACITY
                    )
                    mask_depot = (seq_beam[:, :, -1] == 0) & (
                        (mask_beam == 0).int().sum(-1) > 0
                    )
            ...
            for beam_index in range(beam_size):
                if not self.is_vrp and not self.is_op and not self.is_pctsp:
                    ...
                    d = input.gather(
                        1, seq_beam[:, beam_index, 1:].unsqueeze(-1).expand_as(input)
                    )
                    costs.append(
                    ...
                    d = loc_with_depot.gather(
                        1,
                        seq_beam[:, beam_index, 1:, None].expand(
                            batch_size, seq_beam.size(2) - 1, loc_with_depot.size(-1)
                        ),
                    )
                    costs.append(
                        (d[:, 1:] - d[:, :-1]).norm(p=2, dim=2).sum(1)
                ...
                torch.cat(
                    [
                        ...
                        torch.zeros(
                            batch_size,
                            beam_size,
                            max_length - seq_beam.size(-1),
                            device=seq_beam.device,
                            dtype=seq_beam.dtype,
                        ),
                    ],
                    dim=-1,
                )
                for seq_beam in seq_beam_list
            ]
    ...
    def _get_log_p(
        ...
        used=None,
        remaining=None,
        demand_with_depot=None,
        normalize=True,
    ):
        # Compute query = context node embedding
        query = fixed.context_node_projected + self.project_step_context[path_index](
            self._get_parallel_step_context(fixed.node_embeddings, seq, used, remaining)
        )

        # Compute keys and values for the nodes
        glimpse_K, glimpse_V, logit_K = self._get_attention_node_data(
            fixed, demand_with_depot
        )
    ...
    def _get_parallel_step_context(
        self, embeddings, seq, used, remaining, from_depot=False
    ):
        """
        ...
        if self.is_vrp:
            ...
            if num_steps == 0:
                ...
                return embeddings.gather(
                    1,
                    torch.cat((seq[:, 1, None], current_node), 1)[:, :, None].expand(
