...
class GraphAttentionEncoder(nn.Module):
    def __init__(
        ...
        self.layers = MultiHeadAttentionLayer(
            n_heads, embed_dim, n_layers - 1, feed_forward_hidden, normalization
        )
        self.attention_layer = MultiHeadAttention(
            n_heads, input_dim=embed_dim, embed_dim=embed_dim, last_one=True
        )
    ...
    def change(self, attn, V, h_old, mask, is_tsp=False):
        ...
        attn = (1 - mask.float()).view(1, batch_size, 1, graph_size).repeat(
            n_heads, 1, graph_size, 1
        ) * attn
        if is_tsp:
            attn = attn / (
                torch.sum(attn, dim=-1).view(n_heads, batch_size, graph_size, 1)
            )
        else:
            attn = attn / (
                torch.sum(attn, dim=-1).view(n_heads, batch_size, graph_size, 1) + 1e-9
            )
        heads = torch.matmul(attn, V)
